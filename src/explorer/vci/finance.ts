...
export class VCIFinance {
   ...
   * ```
   */
  static async getCompanyFinancialRatio(): Promise<Array<any>> {
...
`;
    function mapRawFlightInfo(raw: any): any {
      for (const key of Object.keys(raw)) {
        const newKey = MapToBaseFinancialStatement[key];
        if (newKey) {
          raw[newKey] = raw[key];
          delete raw[key];
        }
    ...
    return mapRawFlightInfo(data.CompanyFinancialRatio.ratio[0]);
  }
}
